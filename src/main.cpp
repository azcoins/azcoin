...
#if defined(NDEBUG)
# error "Azcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdeb152be8ab8e278a50f8dafdb74a072a1f974faa3e51adfdc1fe61e82c3f42e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Azcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Azcoin Signed Message:\n";
...
{
    // Azcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Azcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Azcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Azcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Azcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Azcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd08b3f0671aa0f85c18a9de3e20abb48d42f66bdc63c1b6aa1175219c6bfc440");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jean Claude Van Damme will kick them hard";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0419ad8ad9260e456ba4c9c7c4a6649e23ebff14627488c24bfcef42121519d2ea34c6c54f6c52b3d5a3a28b397a94c55ce8cd305271960feea96e33653002ea30") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517690915;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084572170;

        if (fTestNet)
        {
            block.nTime    = 1517690550;
            block.nNonce   = 385737641;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd85a01e899072b92a1624849a97ed4b73f2417963228690b43d235dfe1dc1b11"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xba, 0xd3 }; // Azcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AzcoinMiner(CWallet *pwallet)
{
    printf("AzcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("azcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AzcoinMiner, pwallet));
}
